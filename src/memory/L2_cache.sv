...
module l2_cache #(
    ...
    initial begin
        if (DATA_WIDTH < 32 || (DATA_WIDTH % 8) != 0 ||
            ...
            $fatal(1, "l2_cache requires a power-of-two DATA_WIDTH >= 32 that divides LINE_SIZE");
        if ((LINE_SIZE & (LINE_SIZE - 1)) != 0 ||
            CACHE_SIZE % (LINE_SIZE * NUM_WAYS) != 0 || NUM_SETS < 2 ||
            (NUM_SETS & (NUM_SETS - 1)) != 0 || TAG_BITS < 1)
            $fatal(1, "l2_cache requires power-of-two line/set geometry and a positive tag width");
    end
